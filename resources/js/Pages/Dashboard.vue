...
const updateClock = () => {
    const now = new Date();
    currentTime.value = now.toLocaleTimeString(usePage().props.locale, {
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit",
        hour12: true,
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-8 max-w-7xl mx-auto sm:px-6 lg:px-8 animate-fade-in-up">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 auto-rows-min">
                ...
                <Card
                    variant="glass"
                    class="lg:col-span-3 !mt-0 relative overflow-hidden group"
                >
                ...
                >
                    ...
                    >
                        <!-- Live Clock Display -->
                        <div
                            class="text-4xl font-bold text-white mb-4 font-mono tracking-wider"
                        >
                            {{ currentTime }}
                        </div>

                        <div
                            class="w-20 h-20 rounded-full flex items-center justify-center mb-4 shadow-[0_0_30px_rgba(220,38,38,0.3)] transition-all duration-500"
                            :class="
                                attendance_status === 0
                        ...
                        <!-- Show clock-in time when signed in -->
                        <p
                            v-if="attendance_status !== 0 && clock_in_time"
                            class="text-emerald-400 text-sm font-semibold mb-1"
                        >
                            {{ __("In") }}: {{ clock_in_time }}
                        </p>

                        <!-- Show clock-out time when signed off -->
                        <p
                            v-if="attendance_status === 2 && clock_out_time"
                            class="text-red-400 text-sm font-semibold mb-2"
                        >
                            {{ __("Out") }}: {{ clock_out_time }}
                        </p>
